...
def birdnet_embeddings(
    ...
    log.info(f"Extracting embeddings for {len(files_df)} files")

    ddf = (
        b.map(embed)
        .filter(len)
        .flatten()
        .to_dataframe(meta=embed_meta())
    )
    ...
    log.info(f"Queued BirdNET embeddings, will persist to {outfile}")
...
def get_base_parser():
    ...
    parser.set_defaults(func=main, **{
        ...
        "outfile": "/".join([os.environ.get("DATA_PATH", "/data"), "birdnet_embeddings_table.parquet"]),
...
def register_subparser(subparsers):
    parser = subparsers.add_parser(
        "birdnet_embeddings",
        help="Extract BirdNET feature embeddings",
