...
def copy_except_audio(d: Dict):
    return dict([k, d[k]] for k in set(list(d.keys())) - set(['audio']))
...
def extract_features_from_audio(audio_dict: Dict, frame_length: int = FRAME_LENGTH, hop_length: int = HOP_LENGTH,
    ...
    for feature in Features:
        comp = feature.compute(audio, frame_length=frame_length, hop_length=hop_length, n_fft=n_fft, sr=audio_dict.get('sr'), **kwargs)
