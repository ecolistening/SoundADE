...
class Kilpisjarvi(Dataset):
    FILE_PATH_REGEX = r"(?P<location>K\d+)/Data/(?P<recorder>SMA\d+)_(?P<timestamp>\d+_\d+)\.wav"
    ...
    def filename_metadata(df: pd.DataFrame, filename_column='path') -> pd.DataFrame:
        ...
        df = df[filename_column].str.extract(self.FILE_PATH_REGEX, expand=True)
        ...
        try:
            ...
            logging.warning(f'Error in processing metadata for {df[filename_column]}')
            df['location'] = ''
            df['timestamp'] = datetime.now()

        # Reassemble
        return df.loc[:, :filename_column].join(df.loc[:, 'location':'timestamp']).join(
            df.loc[:, filename_column:].iloc[:, 1:])
